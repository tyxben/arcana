...
- **`src/arcana/multi_agent/team.py::TeamOrchestrator` + `AgentRole` enum** (`src/arcana/contracts/trace.py`). The amendment's Practical Consequence in §103-105 flagged this for "pre-v1.0.0 cleanup," but v1.0.0 was cut (commit `bc15a51`, 2026-05-01) before this amendment was reviewed, and `Runtime.team()` (which the original cleanup language referred to) is a *different* surface from `TeamOrchestrator`. `TeamOrchestrator` is a Planner→Executor→Critic prescribed-topology orchestrator that violates Principle 8 + Amendment 2 + this amendment as a single piece. `arcana.multi_agent.*` is in the *internal — not stable* tier per `specs/v1.0.0-stability.md` §2, so removal does not require a major bump. Treat as a v1.x deprecation cycle: deprecate → next minor → remove.
  - **Step 1 (2026-05-03): DeprecationWarning landed.** `TeamOrchestrator`, `RoleConfig`, and `MessageBus` now emit `DeprecationWarning` on construction with migration text pointing at `runtime.collaborate()` and `Channel`. Migration recipe added to `docs/guide/multi-agent.md`. CHANGELOG `[Unreleased]` notes the deprecation. `tests/test_multi_agent.py` filters the warning so the suite stays clean during the deprecation window.
  - **Step 2 (next v1.x minor): physical removal.** Delete the three classes, the test file, and the `arcana.multi_agent.__init__` re-exports. Same commit also deprecates `AgentRole` and converts `TraceEvent.role` to `agent_name: str | None` with a backward-compat shim that maps any remaining role-addressed code to a name-addressed equivalent.
  - **Step 3 (subsequent minor or v2.0): remove the `AgentRole` shim.**
