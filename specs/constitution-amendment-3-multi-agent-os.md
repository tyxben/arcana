...
- **`src/arcana/multi_agent/team.py::TeamOrchestrator` + `AgentRole` enum** (`src/arcana/contracts/trace.py`). The amendment's Practical Consequence in §103-105 flagged this for "pre-v1.0.0 cleanup," but v1.0.0 was cut (commit `bc15a51`, 2026-05-01) before this amendment was reviewed, and `Runtime.team()` (which the original cleanup language referred to) is a *different* surface from `TeamOrchestrator`. `TeamOrchestrator` is a Planner→Executor→Critic prescribed-topology orchestrator that violates Principle 8 + Amendment 2 + this amendment as a single piece. `arcana.multi_agent.*` is in the *internal — not stable* tier per `specs/v1.0.0-stability.md` §2, so removal does not require a major bump. Treat as a v1.x deprecation cycle: deprecate → next minor → remove.
  - **Step 1 (2026-05-03): DeprecationWarning landed (commit `51ea6da`).** `TeamOrchestrator`, `RoleConfig`, and `MessageBus` emitted `DeprecationWarning` on construction with migration text pointing at `runtime.collaborate()` and `Channel`. Superseded by Step 2 in the same release; the deprecation cycle was effectively zero-window because the classes had not been published to PyPI in any v1.x release.
  - **Step 2 (2026-05-03): physical removal landed.** Deleted `src/arcana/multi_agent/team.py`, `src/arcana/multi_agent/message_bus.py`, `tests/test_multi_agent.py`, the orphan `arcana.contracts.multi_agent.AgentMessage` / `CollaborationSession` / `HandoffResult` types, and all corresponding `__init__.py` re-exports. CHANGELOG `[Unreleased]` flipped from "Deprecated" to "Removed". `arcana.multi_agent.*` is internal-not-stable per `specs/v1.0.0-stability.md` §2; the zero-window justification is documented in the CHANGELOG entry.
  - **Step 3 (partial — same release): `AgentRole` documented as vestigial.** The enum survives unchanged because `TraceEvent.role: AgentRole` is a field on a stable name, and replacing it would be a stable-surface break per `specs/v1.0.0-stability.md` §5. The enum's docstring now states the `PLANNER` / `EXECUTOR` / `CRITIC` members are kept only for historical trace-file parsing, and that replacement of the field with `agent_name: str` is queued for v2.0.
  - **Step 4 (v2.0 work): replace `TraceEvent.role` with `agent_name: str | None`** and remove `AgentRole` entirely. This is a stable-surface break and lives in the v2.0.0 cycle. Track in `specs/v2.0.0-removals.md` when that file is created.
