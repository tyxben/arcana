...
| P3 | Add session bundle and normalized wire trace CLI. **Session-bundle list/show done** (Phase 4 work-item 1, first slice): `TraceReader.list_bundles()`/`read_bundle()` group run JSONLs by the `bundle_id` metadata stamped in Phase 3, correlating `source_agent` + `delegated_by_run_id` parent links and aggregating tokens/cost; CLI `arcana trace bundles` + `arcana trace bundle <id>` (Rich tables, `--json`). **Pending**: bundle `export <id> --out .zip`, human-readable per-turn timeline, normalized wire trace, and the `arcana mcp/skills/chat` CLI surfaces. | `cli/main.py`, `trace/reader.py` | `tests/test_trace_bundle.py` |
