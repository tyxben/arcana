...
| P1 | Trace metadata for permission **(done — `permission_decision` + `provenance` keys)** and protocol-discovery decisions **(done — `PROTOCOL_DISCOVERY` + `ProtocolDiscoveryRecord` for MCP initial discovery, refresh, ignore, and failure)**; guardrail decisions have a trace contract **(`GUARDRAIL_DECISION` + `GuardrailDecisionRecord`)**, with runtime emission deferred until lifecycle hooks / guardrails exist. | `contracts/trace.py`, `mcp/setup.py`, `trace/*` | `tests/test_constitutional_invariants.py`, `tests/test_mcp.py`, `tests/test_mcp_dynamic_discovery.py` |
